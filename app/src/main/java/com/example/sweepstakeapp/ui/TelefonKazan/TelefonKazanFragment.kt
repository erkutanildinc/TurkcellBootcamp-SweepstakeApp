package com.example.sweepstakeapp.ui.TelefonKazan
...
class TelefonKazanFragment : Fragment() {

    private var _binding: FragmentTelefonKazanBinding? = null
    lateinit var listView : ListView
    var list = mutableListOf<DrawEntity>()
    private val binding get() = _binding!!

    companion object {
        fun newInstance() = TelefonKazanFragment()
    }

    private lateinit var viewModel: TelefonKazanViewModel

    override fun onCreateView(
        inflater: LayoutInflater, container: ViewGroup?,
        savedInstanceState: Bundle?
    ): View? {
        _binding = FragmentTelefonKazanBinding.inflate(inflater, container, false)
        val root: View = binding.root
        listView = binding.telefonKazanListView
        val policy = StrictMode.ThreadPolicy.Builder().permitAll().build()
        StrictMode.setThreadPolicy(policy)

        val db: DrawDao = DrawDatabase.getInstance(requireContext())?.drawDao()!!
        list = db.getCategory(4)
    ...
    override fun onActivityCreated(savedInstanceState: Bundle?) {
        ...
        viewModel = ViewModelProvider(this).get(TelefonKazanViewModel::class.java)
